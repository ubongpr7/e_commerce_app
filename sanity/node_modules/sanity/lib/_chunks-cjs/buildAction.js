...
require("./runtime.js");
var shouldAutoUpdate = require("./shouldAutoUpdate.js"), readModuleVersion = require("./readModuleVersion.js"), timing = require("./timing.js");
...
async function buildSanityApp(args, context, overrides) {
  const timer = timing.getTimer(), {
    ...
    flags,
    cliConfig
  }), installedSdkVersion = await readModuleVersion.readModuleVersion(context.workDir, "@sanity/sdk-react");
  if (!installedSdkVersion)
    throw new Error("Failed to find installed @sanity/sdk-react version");
  const coercedSanityVersion = semver__default.default.coerce(installedSdkVersion)?.version;
  ...
  if (autoUpdatesEnabled) {
    ...
    const result = await shouldAutoUpdate.compareStudioDependencyVersions(autoUpdatesImports, workDir);
...
Including the following environment variables as part of the JavaScript bundle:`), envVarKeys.forEach((key) => output.print(`- ${key}`)), output.print(""));
  ...
  autoUpdatesEnabled && (importMap = {
    imports: {
      ...await shouldAutoUpdate.buildVendorDependencies({
        cwd: workDir,
        outputDir,
        basePath
      }),
